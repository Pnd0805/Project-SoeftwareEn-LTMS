...
| C07  | `GET /tournaments/:id`                     | —     | รายละเอียด · `completed` เห็นได้ทุกคนเหมือน `public` (B1 3-ก) + คืน `championTeamId`, `completedAt` · สถานะอื่น → เห็นได้เฉพาะ**ผู้ยื่นคำขอ** (ทุกสถานะรวม `pending_approval`/`rejected`/`completed` — แก้ 20 ก.ย. FE-c17b) และ ADM ในขอบเขต ไม่งั้น **404** | —                                                                                                                                                                                                       | `{ ...ข้อมูลเต็ม, organizer, approvedTeamCount }`                                                                              |

...
| M05c | `GET /me/matches` | Auth | **20 ก.ย.** แมตช์ของฉันทั้ง 2 บทบาท — ผู้เล่น (ทีมที่ฉันเป็นสมาชิก) + กรรมการ (รับแมตช์แล้ว, = F04b) เรียงตามเวลา | `?upcoming=true` (ตัด completed) · `?role=player\|referee` | `{ items: [{id, role, myTeamId, tournament:{id,name,sportTypeId}, round, teamA, teamB, scheduledTime, scheduledEndTime, venue, mode, status}] }` |

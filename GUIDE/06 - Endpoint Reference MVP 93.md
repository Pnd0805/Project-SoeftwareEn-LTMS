...
| C13  | `POST /tournaments/:id/publish`            | ORG   | `private → public` · **BR-10 = F14 coverage ต้อง `uncovered` ว่าง** (ต้องคุยทีม Tournaments)                                                               | —                                                                                                                                                                                                       | `{ id, status:'public' }` / **409** `REFEREES_INCOMPLETE`                                                                      |
...
| M06 | `PATCH /matches/:id/schedule` | ORG | ตั้งเวลา+สนาม · **ตรวจทับซ้อนเป็นช่วงเวลา** (ทีมเดียวกัน/สนามเดียวกัน) · แก้ได้เฉพาะแมตช์ `scheduled` · กรรมการเวลาทับ = ไม่ block (GUIDE/11 Q6) | `scheduledTime, scheduledEndTime, venue` (จบ > เริ่ม) | เหมือน M05 / **409** `SCHEDULE_CONFLICT` + `conflictingMatchId` \| `MATCH_NOT_CHANGEABLE` |
...
| M10 | `POST /matches/:id/start` | REF ของแมตช์ | `checkin_open → in_progress` · ต้องมีคนเช็คอินขั้นต่ำ | — | `{ id, status:'in_progress' }` / **409** `CHECKIN_NOT_OPEN` \| `INSUFFICIENT_CHECKINS` |
...
| M16 | `POST /uploads/presign` | Auth | ขอ URL อัปโหลดไฟล์ขึ้น S3 โดยตรง · `checkin_document` = ต้องอยู่ในทีมของแมตช์ + แมตช์ `checkin_open` (กฎเดียวกับ M12) · `soft_filter_document` = แค่ทัวร์ต้องมีจริง | `purpose, contentType, matchId?, tournamentId?` | `{ uploadUrl, objectKey, expiresIn }` / **403** `NOT_IN_APPROVED_ROSTER` / **409** `CHECKIN_NOT_OPEN` |

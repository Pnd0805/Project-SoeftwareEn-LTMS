...
| C13  | `POST /tournaments/:id/publish`            | ORG   | `private → public` · schedule ต้องครบ + active Referee pool ≥ peak concurrent demand                               | —                                                                                                                                                                                                       | `{ id, status:'public' }` / **409** `SCHEDULE_INCOMPLETE` \| `REFEREES_INCOMPLETE`                                              |
...
| M06 | `PATCH /matches/:id/schedule` | ORG | ตั้งเวลาเริ่ม+สิ้นสุด+สนาม · **ตรวจทับซ้อน 3 มิติ** | `scheduledTime, scheduledEndTime, venue` | เหมือน M05 / **409** `SCHEDULE_CONFLICT` + `conflictingMatchId` |

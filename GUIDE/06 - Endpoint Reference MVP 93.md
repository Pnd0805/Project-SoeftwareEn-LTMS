...
| T21 | `GET /teams/:id/join-requests` | TL | คำขอที่รอตอบ · `user` = โปรไฟล์ที่ผู้ขอกรอก (คณะ/ภาค/ชั้นปี/เพศ/ประเภท) **ไม่มี email/contact/address** (กฎ U03) — ดูสถิติต่อที่ U04 | — | `{ items: [{id, user:{id, fullName, avatarUrl, facultyId, departmentId, year, gender, userType}, message, status, createdAt}] }` |

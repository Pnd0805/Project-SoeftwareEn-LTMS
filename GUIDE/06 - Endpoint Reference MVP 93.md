...
| T09 | `POST /teams/:id/invitations` | TL | เชิญเข้าทีม · **20 ก.ย.**: ทีมเต็ม (`sport_types.max_members`) → **409** `TEAM_FULL` → `pending` · **CoI**: ORG/กรรมการของทัวร์ที่ทีมสมัครอยู่ เข้าทีมไม่ได้ (409 `TEAM_CONFLICT_OF_INTEREST`) · **B6 roster lock (19 ก.ย.)**: ทีมที่มีใบสมัคร `approved` ในทัวร์ที่ยังไม่จบ → **409** `ROSTER_LOCKED` + `tournamentId` ต้องถอนตัว (P08) ก่อน | `invitedUserId` | **201** `{ id, invitedUserId, status:'pending', expiresAt }` |
...
| T13 | `POST /invitations/:id/accept` | Auth | รับคำเชิญ · **20 ก.ย.**: ถ้าทีมเต็มไปก่อนระหว่างรอ → **409** `TEAM_FULL` + `{memberCount, maxMembers}` · **BR-05** · **อาจ Forming→Ready** · transaction · **CoI**: ORG/กรรมการของทัวร์ที่ทีมสมัครอยู่ เข้าทีมไม่ได้ (409 `TEAM_CONFLICT_OF_INTEREST`) · **B6 roster lock (19 ก.ย.)**: ทีมที่มีใบสมัคร `approved` ในทัวร์ที่ยังไม่จบ → **409** `ROSTER_LOCKED` + `tournamentId` ต้องถอนตัว (P08) ก่อน | — | `{ teamId, teamReadinessStatus }` |
...
| T19 | `GET /teams` | — | **ค้นหาทีม (20 ก.ย.)** · ทีมที่ลบ/Inactive ไม่โชว์ (ดูผ่านทัวร์เก่าเท่านั้น) · ไม่คืน roster · ทุกแถวมี `memberCount` + `maxMembers` | `?q=&sportTypeId=&visibility=private\|public&page&pageSize` | `{ items: [TeamDto เหมือน T03], pagination }` |
...
| C09  | `POST /tournaments/:id/amendment-requests` | ORG   | ขอแก้ข้อมูลสำคัญ → `pending` · รับ `eligibilityRules` ด้วย (ทางเดียวที่แก้กฎหลังผ่านอนุมัติ) — ได้เฉพาะยังไม่เปิดรับสมัครและยังไม่มีใบสมัคร ไม่งั้น **409** `ELIGIBILITY_LOCKED`                                                                                      | `requestedChanges` (JSON)                                                                                                                                                                               | **201** `{ id, status:'pending' }`                                                                                             |
...
| C17b | `PUT /tournaments/:id/eligibility-rules`   | ORG   | **แทนที่กฎคุณสมบัติทั้งชุด** (20 ก.ย. Q1-ค) · เฉพาะ `pending_approval` — ผ่านอนุมัติแล้ว **409** `USE_AMENDMENT_REQUEST` (ไป C09) · เปิดรับ/มีใบสมัครแล้ว **409** `ELIGIBILITY_LOCKED` | `{ rules: [{type:'faculty'\|'year', value}] }` (ว่าง = ไม่จำกัด) | `{ items: [{ruleType, ruleValue}] }` |
...
| M05 | `GET /matches/:id` | — | รายละเอียดแมตช์ · ฟิลด์ผลสรุปชุดเดียวกับ M04 · **B8**: `roomCode` (แมตช์ online) เห็นเฉพาะสมาชิกทีมในแมตช์/กรรมการของแมตช์/ORG (ส่ง token) ไม่งั้น null | — | `{ id, tournamentId, round, teamA, teamB, scheduledTime, scheduledEndTime, venue, checkinOpenAt, status, mode, nextMatchId, loserNextMatchId, resultStatus, score, outcome, roomCode }` |

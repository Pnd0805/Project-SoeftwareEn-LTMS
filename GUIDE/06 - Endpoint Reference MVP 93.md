...
| รหัส | Method + Path | Auth | ทำอะไร | รับ | คืน |
|---|---|---|---|---|---|
| P01 | `POST /tournaments/:id/applications` | TL | สมัครแข่ง · **Hard Filter อัตโนมัติ** · BR-04/08/09 · ต้อง ORG เปิดรับสมัคร (C15) **และ** อยู่ใน `registrationStart–registrationEnd` · ทีมต้องกีฬาเดียวกับทัวร์ · **CoI**: สมาชิกทีมเป็น ORG/กรรมการของทัวร์นี้ไม่ได้ | `teamId` | **201** `{ id, status:'pending', hardFilterPassed:true }` / **409** `REGISTRATION_CLOSED` (ข้อความบอกว่ายังไม่เปิด / ยังไม่ถึง / หมดช่วง), `SPORT_TYPE_MISMATCH`, `TEAM_CONFLICT_OF_INTEREST` + `conflicts[{userId, role}]` / **422** `HARD_FILTER_FAILED` + `details[]` |

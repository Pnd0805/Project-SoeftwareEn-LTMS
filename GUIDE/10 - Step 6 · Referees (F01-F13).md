...
| F-13 | เก็บเอกสารที่ ref ส่งให้ admin ไว้ที่ไหน | **migration 004** — `external_verification_docs JSON` + `external_rejection_reason TEXT` ใน `tournament_referees` (แบบเดียวกับ ORG ภายนอกใน `tournaments`) · เป็น array S3 key จาก M16 · **ล้างเป็น NULL ทันทีที่ admin ตัดสิน** (PDPA) · ส่งได้ตอน F05 (`docs`) หรือ F15 |
| F-14 | "เอกสารที่ admin ระบุไว้" กำหนดที่ไหน | **ค่าคงที่ทั้งระบบ ยังไม่ enforce ฝั่ง backend** — แนะนำ 2 รูป: บัตรประชาชน + selfie ถือบัตร (ยืนยัน "เป็นคนจริง" เท่านั้น ความสามารถตัดสินกีฬาเป็นความรับผิดชอบของ ORG ที่เชิญ) · backend รับ 1–5 key · FE เป็นคนบอกว่าต้องส่งอะไร |
| F-15 | admin ไม่อนุมัติคนนอก แล้วยังไงต่อ | **AR03 + `reason` บังคับ** → สถานะ `rejected_by_admin` · ไม่มีการแจ้ง ORG อัตโนมัติ (ไม่มี notification) ORG เห็นจาก F02/F14 · ORG **เชิญคนเดิมซ้ำได้** เป็นแถวใหม่ → accept แล้ว pending อีกครั้ง (ผลที่ถูก reject ไม่ถูกก็อป) |
| F-17 | admin reject เพราะเอกสารไม่ชัด ทัวร์โดนไปด้วย | **แยกปุ่ม AR04 request-docs** (15 ก.ย. 2026) → `needs_docs` + ข้อความ · ทัวร์ที่รอยังรอต่อ · user ส่งใหม่ผ่าน U12 ครั้งเดียวถึงทุกทัวร์ · reject (AR03) = ไม่ผ่านจริงเท่านั้น · การตรวจเป็น "ต่อคน": accept ทัวร์ B ระหว่างทัวร์ A ยัง pending → ร่วมการตรวจเดิม ไม่ต้องส่งซ้ำ · admin เห็นคิว 1 รายการต่อคน |
| F-18 | "1 ปี" หมดอายุระหว่างทัวร์ทำไง | **ไม่มีผลกับทัวร์ที่ accept ไปแล้ว** (15 ก.ย. 2026) — 1 ปี = อายุของการนำผลเก่าไปใช้ accept ทัวร์ใหม่ (เช็คครั้งเดียวตอน accept, นับจาก `approved_at` ครั้งแรก ไม่ต่ออายุเมื่อใช้) ไม่ใช่อายุของสิทธิ์ในทัวร์ที่รับไปแล้ว → กรรมการไม่หลุดกลาง bracket · ถ้าต้องการเข้มขึ้น: เช็คตอน accept ว่า approval ครอบคลุมถึง `event_end_date` ไหม (ยังไม่ทำ) |
| F-16 | ยืนยันตัวตนต่อทัวร์ vs ต่อคน | **ก็อป 1 ปี** — `findRecentApproval`: ถ้า user เคย approved ในทัวร์ใดก็ได้ภายใน 1 ปี F05 เขียนแถวใหม่เป็น approved ทันที (ก็อป approved_by/at) · เลือกแบบนี้แทนย้ายไป `users` เพราะไม่อยากแตะ schema ข้ามทีม (ORG ภายนอกก็เป็นต่อทัวร์เหมือนกัน) · ถ้าจะทำต่อคนในอนาคต ให้ย้ายพร้อม ORG ภายนอก |
| F-19 | กรรมการมีส่วนได้เสีย (รายงาน FE ข้อ 7) | **กันทุกกรณี (มติ 18 ก.ย. 2569)**: F01 ปฏิเสธ ORG เอง (`ORGANIZER_CANNOT_BE_REFEREE`) และคนที่มีชื่อในทีมที่สมัครทัวร์นี้ pending/approved แม้ไม่ได้ลงแข่ง (`REFEREE_CONFLICT_OF_INTEREST`) · P01 ปฏิเสธทีมที่มีสมาชิกเป็น ORG/กรรมการ (คำเชิญ pending/accepted) ของทัวร์นี้ (`TEAM_CONFLICT_OF_INTEREST`) = กันสมัคร**หลัง**ถูกเชิญด้วย · ประตูที่ 3 ปิดแล้ว: T09 เชิญเข้าทีม / T13 กดรับ → ถ้าทีมสมัครทัวร์ที่คนนี้เป็น ORG/กรรมการอยู่ → 409 `TEAM_CONFLICT_OF_INTEREST` + `{tournamentId, role}` (คำเชิญคง pending ไว้) | ☑ |

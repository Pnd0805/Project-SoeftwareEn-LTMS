...
| **double elimination** | — | ทีมที่ถอน "แพ้บาย" ถูกวางลงสายล่างตามปกติ → แมตช์สายล่างนั้นบายต่อให้อีกฝั่ง ไล่เป็นลูกโซ่จนไม่เจอทีมนี้ · **ถอนทั้งสองทีม** (สายล่าง หรือคู่ที่มาทีหลังก็ถอน) = ไม่มีใครให้ชนะ → **แพ้ทั้งคู่อัตโนมัติ** (ใบผล `walkover` `winner_team_id=NULL`, `lost+1` ทั้งคู่, audit `reason='both_withdrawn'`) ไม่มีใครเดินสาย → ช่องรอบถัดไปว่างถาวร ทีมที่รออยู่บายผ่าน (dead slot) — ORG ไม่ต้องกดอะไร |

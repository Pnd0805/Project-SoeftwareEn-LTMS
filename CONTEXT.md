...
- **Tournament scope** — ระดับหน่วยงาน/ขอบเขตการจัด Tournament; Current MVP API เปิดระดับภาควิชา (`department`) และคณะ (`faculty`) โดย eligibility ของผู้สมัครเป็นกฎแยกต่างหาก
- **University-wide tournament** — Tournament ระดับมหาวิทยาลัย (`university`); domain/database รองรับแต่ Current MVP API/UI ยัง deferred
...
- **Referee capacity for publication** — จำนวน active Tournament Referee ต้องไม่น้อยกว่า peak concurrent referee demand จาก planned Match schedule; online ใช้ขั้นต่ำ 1 คน/Match และ on-site ใช้ขั้นต่ำ 2 คน/Match
...
- **Age eligibility** — ผู้ใช้มีวันเกิด (`birthDate`/`birth_date`) ส่วน Tournament มีขอบเขตอายุเป็นตัวเลข (`minAge`/`maxAge`); ระบบคำนวณอายุเต็มปี ณ `registrationEnd`/`registration_end`

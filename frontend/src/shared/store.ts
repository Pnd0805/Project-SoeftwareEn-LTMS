...
/* ───────── fixtures ───────── */
export function saveFixture(matchId: string, input: { kickoff: string; venue: string; pin: string; refs: string[] }) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.kickoff = input.kickoff ? new Date(input.kickoff).toISOString() : ''
  m.venue = input.venue
  m.refs = input.refs
  commit()
  toast('Fixture saved')
...
/* ───────── results ───────── */
export function enterResult(matchId: string, by: string, input: {
  sa: number; sb: number; decider?: { a: number; b: number; kind: string } | null
  stats?: Match['stats']; teamStats?: Match['teamStats']
}) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  const tr = state.tournaments.find(t => t.id === m.tour)!
  if (input.sa === input.sb && !input.decider && formatOf(tr) !== 'roundrobin') {
    toast(`A knockout match cannot end level — record the ${'decider'} beside the score`, 'crit')
    return
  }
  m.sa = input.sa
  m.sb = input.sb
  m.decider = input.decider ?? null
  m.stats = input.stats ?? m.stats
  m.teamStats = input.teamStats ?? m.teamStats
  m.enteredBy = by
  m.status = 'pending'
  const win = winnerId(m)
  const signer = tr.channel === 'onsite'
    ? state.teams.find(t => t.id === win)?.leader
    : tr.referees[0]
  notify(signer, `A result in ${tr.name} is waiting on your confirmation.`, `/m/${m.id}`)
  commit()
  toast('Recorded — waiting on confirmation')
}

export function confirmResult(matchId: string, by: string, asOrganizer = false) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.status = 'confirmed'
  m.confirmedBy = by
  m.confirmedByOrg = asOrganizer
  advance(m)
  const tr = state.tournaments.find(t => t.id === m.tour)
  notifyAll([m.a, m.b].flatMap(tid => state.teams.find(t => t.id === tid)?.members ?? []),
    `A result in ${tr?.name} was confirmed.`, `/m/${m.id}`)
  commit()
  toast(asOrganizer ? 'Confirmed — recorded as your decision, with your name on it' : 'Result confirmed')
}

export function disputeResult(matchId: string, by: string, teamId: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  const tr = state.tournaments.find(t => t.id === m.tour)
  if (tr?.channel === 'online') { toast('There is no dispute on an online match — the referee checks it instead', 'crit'); return }
  m.status = 'disputed'
  m.disputedBy = by
  m.disputedTeam = teamId
  notify(tr?.organizer, `${state.teams.find(t => t.id === teamId)?.name} disputed a result in ${tr?.name}.`, `/m/${m.id}`)
  commit()
  toast('Disputed — the organizer settles it', 'warn')
}

export function resolveDispute(matchId: string, by: string, sa: number, sb: number) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.sa = sa
  m.sb = sb
  ...
  advance(m)
  commit()
  toast('Settled — your decision is final')
}

export function reopenResult(matchId: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.status = 'pending'
  m.confirmedBy = null
  commit()
  toast('Reopened for review', 'warn')
}

export function saveReplay(matchId: string, url: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.replay = url
  commit()
  toast('Replay link saved')
}

export function saveLineup(matchId: string, teamId: string, starters: string[], subs: string[]) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.lineup = { ...(m.lineup || {}), [teamId]: { starters, subs } }
  commit()
  toast('Lineup named')
}

export function checkIn(matchId: string, userId: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m || m.checkedIn.includes(userId)) return
  m.checkedIn.push(userId)
  commit()
  toast('Checked in')
}

export function markIneligible(matchId: string, userId: string, by: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.checkedIn = m.checkedIn.filter(id => id !== userId)
  const tr = state.tournaments.find(t => t.id === m.tour)
  notify(tr?.organizer, `${state.users.find(u => u.id === by)?.name} marked ${state.users.find(u => u.id === userId)?.name} ineligible.`, `/m/${m.id}`)
  commit()
  toast('Reported to the organizer', 'warn')
}

export function setRoomCode(matchId: string, code: string) {
  const m = state.matches.find(x => x.id === matchId)
  if (!m) return
  m.roomCode = code
  commit()
  toast('Room code shared with both squads')
}

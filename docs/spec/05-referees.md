...
## 4. Publication Referee Capacity — Current Team Decision

ก่อน publish ระบบต้องทราบ planned match time ranges จาก bracket/schedule แล้วคำนวณ **จำนวน Referee ขั้นต่ำที่ต้องพร้อมใช้งานพร้อมกันสูงสุด**

นิยาม:

```text
requiredRefereeCapacity =
  max over time t (
    sum requiredRefereeCount(match)
    for every planned match active at time t
  )
```
...
```text
09:00-10:00  M1 requires 2 refs
09:00-10:00  M2 requires 1 ref
09:30-10:30  M3 requires 1 ref

peak concurrent demand = 4
```

ดังนั้น Tournament ต้องมี active referee capacity อย่างน้อย 4 คนสำหรับ schedule นี้ก่อน publication readiness ผ่าน

**ADR-0005 ที่กำหนดขั้นต่ำ 1 คนถูก supersede โดย rule นี้**
...
## 11. Implementation Gap Notes

Current schema/API เดิมยังมีข้อจำกัดเมื่อเทียบกับ spec นี้ เช่น:

- `match_referees` ยังไม่มี assignment response state
- referee change-request storage/endpoint ยังไม่มี
- external verification-document model ยังไม่มี

ข้อเหล่านี้เป็น implementation gaps ไม่ใช่เหตุผลให้ลด requirement โดยอัตโนมัติ

...
- **กันสแปม** (22 ก.ย.): คนละไม่เกิน **5 คอมเมนต์ต่อ 60 วินาที** รวมทุกแมตช์ (นับที่ลบไปแล้วด้วย — ลบแล้วไม่ได้โควตาคืน) → 429 `COMMENT_RATE_LIMITED {retryAfterSeconds}` · นับจาก `match_comments.created_at` ใน DB (ไม่ต้องมีตาราง/แพ็กเกจเพิ่ม · สองคำขอพร้อมกันเป๊ะอาจหลุดได้ 1 อัน ยอมรับได้)

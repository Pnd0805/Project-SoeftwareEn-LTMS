...
- **ทีมถอนตัว** (ตัดสิน 21 ก.ย.): หัวหน้า/ผู้เล่นทีมที่ถอนให้คะแนนไม่ได้อีกหลังถอน (FE ถามให้คะแนนก่อนยืนยันถอนตัว · ความเห็นที่ให้ไว้แล้วยังนับ) · โหวต MVP ได้เหมือนคนนอก · ผู้เล่นทีมที่ถอนไม่เป็นผู้ถูกโหวต
...
## OD-24 — Follows, career and profile totals (C8) — ✅ Resolved 2026-09-21

FE-follows-plus-any-profile: หน้าโปรไฟล์/หน้าทีมมีปุ่มติดตาม ประวัติแยกตามทัวร์ MVP รวม และ Pick'em รวม — ทำงานเฉพาะ mock

- **1 ติดตามอะไร**: ทั้งผู้เล่นและทีม (FE มีปุ่มทั้งสองหน้า) · migration **024** เพิ่ม `follows.followed_team_id` + CHECK ชี้อย่างใดอย่างหนึ่งพอดี (023 เว้นไว้ให้ C5 ตามเอกสารแบ่งงาน) · ติดตามตัวเองไม่ได้ (`CANNOT_FOLLOW_SELF`) · ทีมที่ถูกลบติดตามไม่ได้และไม่ขึ้นในรายการ/ไม่นับ
- **2 ติดตามแล้วได้อะไร**: นับ + รายการเท่านั้น — ยังไม่มีแจ้งเตือนเมื่อคน/ทีมที่ติดตามมีแมตช์ (ต่อยอดด้วย C1 ได้ภายหลัง) · กด/เลิกซ้ำได้ผลเดิม (idempotent)
- **3 career**: สาธารณะ (เหมือน `/users/:id/stats`) · เฉพาะทัวร์ที่คนนี้อยู่ในรายชื่อลงแข่งของใบสมัคร approved (ทีมที่ถอนตัว → ไม่ขึ้น เพราะรายชื่อถูกลบ) · played/wins นับจากผล `verified` เท่านั้น walkover ไม่นับ (ไม่ได้ลงเล่นจริง — กฎเดียวกับ player_profile_stats) · `champion` = ทีมตรงกับ `tournaments.champion_team_id`
- **4 ตัวเลขรวม**: `mvpVotes` = โหวต MVP ที่ได้ทั้งหมด (C6, ไม่นับที่แอดมินลบ) · `pickemPoints` = `users.total_points` (0 จนกว่า C7 จะทำ) · `followerCount` · GET /users/:id เพิ่ม `followerCount, followingCount, isFollowing` (ดูตัวเอง/ไม่ล็อกอิน = false)

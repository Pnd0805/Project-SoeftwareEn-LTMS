...
## OD-26 — ปิดจุดที่แมตช์/ทัวร์ค้างในระบบส่งผล–ยืนยันผล — 🚧 ทยอยทำ (ตัดสินครบ 25–26 ก.ย. 2569)
...
**ยังไม่ทำ (ตัดสินแล้ว)**
- **ก้อน 2** ข้อ 4+2+5: ปุ่ม "จบการแข่งขัน" **แบบบังคับ** (สถานะ `finished`) + `started_at`/`actual_end_time`/`submitted_at` — ต้องบังคับเพราะ MVP รายแมตช์และนาฬิกาของข้อ 6/7 ต้องใช้เวลาจบจริง · กรรมการ **และ** ORG กดได้ (ข้อนี้คือสิ่งที่ทำให้ "บังคับ" ไม่กลายเป็นจุดค้างใหม่) · **มี migration 1 ตัว**
...
- **ก้อน 4** ข้อ 8: ค้านหลังสายเดินแล้ว **ไม่เข้า `disputed`** เป็น "เรื่องร้องเรียน" ที่ไม่เปลี่ยนผล · ยื่นได้เฉพาะก่อนปิดทัวร์ · ORG แนบความเห็นได้ 48 ชม. **ปัดตกไม่ได้** แล้วขึ้นแอดมินอัตโนมัติ · เรื่องผูกกับผลแมตช์ ไม่ใช่ตัว ORG · บันทึกเฉพาะคนยื่นเมื่อแอดมินตัดสินว่าไม่มีมูล · รอเช็คกับ `user_reports` ของ `backend_step9-10` ก่อน จะได้ไม่มีระบบร้องเรียนซ้อนกัน
